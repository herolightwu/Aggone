...
import com.odelan.yang.aggone.Adapter.Stats.ClubAdapter;
...
public class ClubDetailActivity extends BaseActivity {
    ...
    private void refreshActivity(){
        ...
        API.getClubYearSummary(this, user, new APICallback<List<Summary>>() {
            @Override
            public void onSuccess(List<Summary> response) {
                ...
                for (Summary club : response) {
                    ...
                    skills = getSportSkills(Constants.STATS_COACH, club.stats);
                    column = 0;
                ...
                if(response.size() > 0){
                    ...
                    params.height = convertDpToPixelInt(response.size() * 40 + 90, ClubDetailActivity.this);
        ...
        String[] ss = sYear.split("-");
        API.getYearResultSummary(this, user, Integer.valueOf(ss[0]), new APICallback<List<List<Pair<String, Integer>>>>() {
            @Override
